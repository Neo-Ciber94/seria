...
describe("Basic stringify/parse", () => {
    ...
    test("Should stringify/parse array with holes", () => {
        ...
        expect(json).toStrictEqual('["$A1",["$undefined","$$Yatora","$undefined","$undefined","$$Ryuji","$undefined"]]');
    ...
    test("Should stringify/parse map with string keys ", () => {
        const json = stringify(new Map<any, any>([
            ["number", 1],
            ["text", "hello"],
            ["boolean", true],
            ["null", null],
        ...
        expect(json).toStrictEqual('["$Q1",[["$$number",1],["$$text","$$hello"],["$$boolean",true],["$$null",null],["$$undefined","$undefined"]]]');
        const value = parse(json);
        expect(value).toStrictEqual(new Map<string, any>([
            ["number", 1],
            ["text", "hello"],
            ["boolean", true],
            ["null", null],
    ...
    test("Should stringify/parse map with different keys", () => {
        ...
        expect(json).toStrictEqual('["$Q1",[[3,true],["$undefined","$$adios"],["$n-2","$D2000-05-03T04:00:00.000Z"],["$Skey",false]]]');
